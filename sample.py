...
# # Flask-Login user loader for Faculty
...
#         error = 'Invalid email domain. Only woxsen.edu.in emails are allowed.'
#         return jsonify({'error': error})
#     else:
...
#         else:
#             return jsonify({'error': 'Student login failed.'})
...
#     c.execute('INSERT INTO job (Job_id, department, designation, description, pdf_path) VALUES (?, ?, ?, ?, ?)',
#               (job_id, department, designation, description, pdf_path))
#     conn.commit()
#     conn.close()
...
#     if job:
#         # Check if job_id is changing
#         if job_id != int(new_job_id):
#             # Check if the new job_id already exists
#             c.execute('SELECT * FROM job WHERE Job_id = ?', (new_job_id,))
#             existing_job = c.fetchone()
#             if existing_job:
#                 conn.close()
#                 return jsonify({'success': False, 'error': 'New job_id already exists'}), 400

#         # Update the job details
#         c.execute('UPDATE job SET Job_id = ?, department = ?, designation = ?, description = ? WHERE Job_id = ?',
#                   (new_job_id, department, designation, description, job_id))
...
#             pdf_path = os.path.join(app.config['PDF_UPLOAD_FOLDER'], filename)
#             pdf.save(pdf_path)
#             c.execute('UPDATE job SET pdf_path = ? WHERE Job_id = ?', (pdf_path, new_job_id))
